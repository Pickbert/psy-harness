...
export const Config: Schema<JsonRpcConfig> = Schema.object({
  ...
  desktopPetPolicy: Schema.boolean().default(false),
})
...
export function apply(ctx: Context, config: JsonRpcConfig): void {
  ...
  const resolvedConfig = config as JsonRpcConfig & { maxTokensAsSuccess: boolean; desktopPetPolicy: boolean }
  ...
  const server = new HarnessSdkJsonRpcServer(ctx, transport, {
    ...
    desktopPetPolicy: resolvedConfig.desktopPetPolicy,
  })
