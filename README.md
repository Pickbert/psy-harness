...
哈妮丝是一只运行在 macOS 和 Windows 桌面上的原生卡通长毛职业咨询猫，也是一套带安全审批能力的本地 DeepSeek Agent 宿主。她面向中学生、大学生及其他处于学习阶段的用户，帮助认识兴趣、优势与价值观，探索专业、升学和职业方向，比较现实选项并形成行动计划；同时保留随机散步、待机、拖动和点击互动。配置自己的 DeepSeek API Key 后，还可以进行连续生涯规划、分析拖入的资料，并在用户选定的目录内调用辅助能力。

哈妮丝提供的是 AI 教育与职业探索支持，不替学生、家长或老师作决定，也不承诺升学、录取、实习或就业结果。招生政策、院校要求和就业信息应通过官方或可靠来源核实；遇到明确自伤、自杀、伤人或其他紧急危险时，应立即联系当地急救、危机干预资源、学校心理老师、专业人员或可信赖的现实支持者。
...
- 结束本轮生涯规划时，基于本轮对话生成固定八部分模板的 PDF 生涯规划报告，并提示保存位置
...
1. 启动后，右键职业咨询猫或菜单栏/托盘图标，选择“职业咨询模型设置…”。
2. 填写自己的 API Key，并选择 `DeepSeek V4 Flash` 或 `DeepSeek V4 Pro`。
3. 连续快速点击小猫 3 次，或使用全局快捷键打开输入框。
4. 完整 Agent 版第一次对话会要求选择一个长期工作目录。
5. 输入想了解的专业、升学、实习、求职或长期职业问题并按回车；后续再次唤醒输入框可继续本轮生涯规划。
6. 选择“结束本轮生涯规划”后，哈妮丝会先生成 PDF 报告，成功保存后再清空本轮上下文。

打开“职业咨询模型设置”期间，哈妮丝会暂时保持静止，关闭设置窗口后自动恢复陪伴，以便稳定地点击模型下拉框完成选择。

API Key 不会写入项目文件：macOS 使用系统钥匙串，Windows 使用 Windows 凭据管理器。普通对话和 Agent 都直接连接 DeepSeek 官方接口。

完整 Agent 版可在“生涯规划助手 → 生涯规划助手设置…”中管理辅助能力。职业咨询猫人设、信息核实原则与安全规则由应用内置且不可编辑，并同时用于基础生涯咨询和 Harness 生涯规划助手；保存辅助能力设置后会开启新的技术会话上下文，并在条件允许时自动重启生涯规划助手。

### 本轮生涯规划报告

报告固定包含“谈话目标、咨询过程主要内容、兴趣优势与价值偏好、教育与职业方向、选项比较与判断依据、行动计划、行动的监督、支持资源与下一步”八部分，并带有 AI 生涯规划记录免责声明。其中，“行动的监督”会记录本轮共同确认的监督人或自我监督方式、检查或复盘时间、完成标准及调整方式；未形成安排时会明确说明。模型未从对话中获得的信息会标为“本轮对话中未明确提及”。

- macOS：保存到 `~/Documents/哈妮丝生涯规划报告/`，完成后可直接在 Finder 中显示。
- Windows：保存到当前用户“文档/哈妮丝生涯规划报告/”，完成后可打开所在文件夹；系统需要启用 Windows 自带的 `Microsoft Print to PDF`。
- 只有 PDF 成功写入后才会清空本轮生涯规划；生成失败时会保留本轮对话，便于重试。
...
- 不支持旧版二进制 `.xls`。
- 文件会话默认保留 7 天；可从“生涯规划助手”菜单结束资料分析或清除当前生涯资料目录的缓存。

### 等待召唤

默认连续 3 分钟没有点击、拖动、菜单操作或生涯规划对话后，哈妮丝会蹲坐等待。可在“设置陪伴等待时间…”中填写 `0–1440` 分钟；`0` 表示关闭自动等待。设置在重启后保留。
...
- 职业咨询猫人设、学生生涯规划方法、信息核实原则与工作区安全规则均由应用固定提供，不能在生涯规划助手设置中编辑或覆盖。
