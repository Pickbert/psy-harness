...
import { desktopPetPreToolDecision, HarnessSdkJsonRpcServer } from '../src/index.ts'

class FakeTransport implements JsonRpcTransportPeer {
  notifications: { method: string; params?: Record<string, unknown> }[] = []

  constructor(private readonly requestHandler?: (method: string, params: object, signal?: AbortSignal) => Promise<unknown>) {}
...
describe('HarnessSdkJsonRpcServer', () => {
  it('denies destructive and out-of-workspace calls while asking for ordinary mutations', () => {
    const execution = (name: string, args: unknown) => ({ name, arguments: args }) as never
    expect(desktopPetPreToolDecision(execution('read', { file_path: 'README.md' }), '/workspace')).toBeUndefined()
    expect(desktopPetPreToolDecision(execution('write', { file_path: 'notes.md' }), '/workspace'))
      .toMatchObject({ kind: 'ask' })
    expect(desktopPetPreToolDecision(execution('bash', { command: 'swift test', description: 'Run tests' }), '/workspace'))
      .toEqual({ kind: 'ask', reason: 'Run tests' })
    expect(desktopPetPreToolDecision(execution('bash', { command: 'rm -rf build' }), '/workspace'))
      .toMatchObject({ kind: 'deny' })
    expect(desktopPetPreToolDecision(execution('read', { file_path: '../secret' }), '/workspace'))
      .toMatchObject({ kind: 'deny' })
    expect(desktopPetPreToolDecision(execution('bash', { command: 'cat $HOME/.ssh/config' }), '/workspace'))
      .toMatchObject({ kind: 'deny' })
  })
  ...
  it('bridges owned-session approvals to the DesktopPet client and accepts only one-shot decisions', async () => {
    ...
    ;(server as unknown as { sessions: Map<string, unknown> }).sessions.set('main', {})
    const listener = listeners.get('approval/request') as unknown as (
      req: { agent: { session: { id: SessionId } }; callId: string; toolName: string; reason: string },
      next: () => Promise<string>,
    ) => Promise<string>

    const outcome = await listener({
      agent: { session: { id: SessionId('main') } },
      callId: 'call-1',
      toolName: 'bash',
    ...
    expect(requests[0]).toMatchObject({
      method: 'desktopPet/approval.request',
      params: {
        sessionId: 'main',
        callId: 'call-1',
        toolName: 'bash',
        summary: 'command execution requires confirmation',
  ...
  it('resumes a persisted SDK session instead of creating a colliding session', async () => {
    const followup = vi.fn<Agent['followup']>()
    const agent = ({
      id: SessionId('persisted-main'),
      followup,
    } satisfies Pick<Agent, 'id' | 'followup'>) as unknown as Agent
    const handle = { agent, dispose: vi.fn(() => Promise.resolve()) }
    const create = vi.fn(() => Promise.resolve(handle))
    ...
    const ctx = {
      ...
      agents: {
        create,
        resume,
        get: (id: SessionId) => String(id) === 'persisted-main' ? agent : undefined,
      },
      get: vi.fn((name: string) => name === 'sessionPersistence'
        ? { list: () => Promise.resolve([{ id: SessionId('persisted-main'), cwd: '/workspace' }]) }
        : name === 'llm'
          ? { listProviders: () => [{ id: 'test-provider' }] }
          : undefined),
    } as unknown as Context
    const server = new HarnessSdkJsonRpcServer(ctx, new FakeTransport())
    await server.initialize({ cwd: '/workspace', provider: 'test-provider', model: 'test-model' })

    const receipt = await server.prompt({
    ...
    expect(resume).toHaveBeenCalledWith({
      resumeSessionId: SessionId('persisted-main'),
