...
哈妮丝是一只运行在 macOS 和 Windows 桌面上的原生卡通长毛心理咨询猫，也是一套带安全审批能力的本地 DeepSeek Agent 宿主。她以温和、稳定、非评判的方式陪用户梳理情绪、需要、资源与下一步，同时保留随机散步、待机、拖动和点击互动；配置自己的 DeepSeek API Key 后，还可以进行连续咨询、分析拖入的资料，并在用户选定的目录内调用辅助能力。

哈妮丝提供的是 AI 心理支持与自我梳理，不等同于持证心理咨询、心理治疗、医学诊断或紧急援助。遇到明确自伤、自杀、伤人或其他紧急危险时，应立即联系当地急救、危机干预资源、专业人员或可信赖的身边人。
...
- 结束本轮咨询时，基于本轮对话生成固定八部分模板的 PDF 咨询报告，并提示保存位置
...
1. 启动后，右键咨询猫或菜单栏/托盘图标，选择“咨询模型设置…”。
2. 填写自己的 API Key，并选择 `DeepSeek V4 Flash` 或 `DeepSeek V4 Pro`。
3. 连续快速点击小猫 3 次，或使用全局快捷键打开输入框。
4. 完整 Agent 版第一次对话会要求选择一个长期工作目录。
5. 输入想倾诉或梳理的内容并按回车；后续再次唤醒输入框可继续本轮咨询。
6. 选择“结束本轮咨询”后，哈妮丝会先生成 PDF 报告，成功保存后再清空本轮上下文。

打开“咨询模型设置”期间，哈妮丝会暂时保持静止，关闭设置窗口后自动恢复陪伴，以便稳定地点击模型下拉框完成选择。

API Key 不会写入项目文件：macOS 使用系统钥匙串，Windows 使用 Windows 凭据管理器。普通对话和 Agent 都直接连接 DeepSeek 官方接口。

完整 Agent 版可在“咨询助手 → 咨询助手设置…”中管理辅助能力。心理咨询猫人设与风险处置规则由应用内置且不可编辑，并同时用于基础咨询和 Harness 咨询助手；保存辅助能力设置后会开启新的技术会话上下文，并在条件允许时自动重启咨询助手。

### 本轮咨询报告

报告固定包含“本轮咨询主题、情绪与事实摘要、核心需要与价值、已有资源与支持、本轮关键洞察、约定的下一步、风险关注、专业支持建议”八部分，并带有 AI 支持性记录免责声明。模型未从对话中获得的信息会明确标为“本轮对话中未明确提及”，报告不会生成诊断结论。

- macOS：保存到 `~/Documents/哈妮丝咨询报告/`，完成后可直接在 Finder 中显示。
- Windows：保存到当前用户“文档/哈妮丝咨询报告/”，完成后可打开所在文件夹；系统需要启用 Windows 自带的 `Microsoft Print to PDF`。
- 只有 PDF 成功写入后才会清空本轮咨询；生成失败时会保留本轮对话，便于重试。
...
- 不支持旧版二进制 `.xls`。
- 文件会话默认保留 7 天；可从“咨询助手”菜单结束资料分析或清除当前咨询资料目录的缓存。

### 等待召唤

默认连续 3 分钟没有点击、拖动、菜单操作或咨询后，哈妮丝会蹲坐等待。可在“设置陪伴等待时间…”中填写 `0–1440` 分钟；`0` 表示关闭自动等待。设置在重启后保留。
...
- 心理咨询猫人设、风险转介要求与工作区安全规则均由应用固定提供，不能在咨询助手设置中编辑或覆盖。
