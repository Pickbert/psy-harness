...
class SingleExeBuild {
  ...
  async verifyClosure(): Promise<void> {
    if (process.env.DSH_DESKTOPPET_SKIP_CLOSURE_VERIFY === '1') {
      console.log('build-exe-for-python-sdk: skipping runtime dependency closure verification by explicit environment override')
      return
    }
